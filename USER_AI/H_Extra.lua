...
-- BLUEPRINT COMBO LOGGING (node-based visual combos from GUI)
LogEnable["BP_COMBO"] = 1            -- Blueprint combo execution and node traversal
...
LogEnable["AAI_STATE"] = 1           -- State machine transitions
...
function OnInit()
	...
	-- Log key configuration flags for runtime visibility
	TraceAI(string.format(
		"[INIT] Config: AggroHP=%s SuperPassive=%s UseAttackSkill=%s UseSkillOnly=%s ComboEnabled=%s Chase=%s Attack=%s FollowStayBack=%s",
		tostring(AggroHP), tostring(SuperPassive), tostring(UseAttackSkill), tostring(UseSkillOnly), tostring(ComboEnabled), tostring(ComboRunDuringChase), tostring(ComboRunDuringAttack), tostring(FollowStayBack)
	))
...
--########################################
-- BLUEPRINT COMBO TACTICS INTEGRATION
--########################################
-- Load ComboTactics from Blueprint Editor (guarded by BlueprintComboEnabled)
if BlueprintComboEnabled == 1 then
    local candidates = {
        ConfigPath.."data/ComboTactics_Heal_Sustain_DPS_Combo.lua",                          -- Your exported combo (from KimiAIConfig GUI)
        -- ConfigPath.."data/ComboTactics_AttackState_DPS_Sustain.lua",  -- Example: DPS + healing rotation
        -- ConfigPath.."data/ComboTactics_TestExport.lua",               -- Fallback test combo
    }
    local loaded = false
    for _, path in ipairs(candidates) do
        local ok, _ = pcall(dofile, path)
        if ok then
            TraceAI("[INIT] Loaded blueprint combo: "..path)
            loaded = true
            break
        end
    end
    if not loaded then
        TraceAI("[INIT] No blueprint combo file found (export from Combo Tactics tab in KimiAIConfig)")
    end
else
    TraceAI("[INIT] Blueprint combos disabled (BlueprintComboEnabled=0 in H_Config.lua)")
end

--########################################
-- END OF H_EXTRA.LUA
--########################################
-- (Note: Additional route and movement configuration should be defined
-- within properly scoped tables. Stray table entries below were removed.)
...
-- (OnInit is defined above; avoid duplicate definitions)
