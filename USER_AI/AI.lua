-----------------------------
-- Dr. Azzy's Homunculus AI
-- Written by Dr. Azzy of iRO Loki
-- Permission granted to distribute in unmodified form
-- Please contact me via the iRO Forums if you wish to modify
-- so that we can work together to extend and improve this AI.
-- Customized for the 'Return of Morroc' server by Nathan.
-----------------------------
Version="1.6"
ErrorCode=""
ErrorInfo=""
LastSavedDate=""
TactLastSavedDate=""
TypeString="H"

dofile("./AI_sakray/USER_AI/Const_.lua")
dofile("./AI_sakray/USER_AI/H_SkillList.lua") 
dofile("./AI_sakray/USER_AI/Defaults.lua")
dofile("./AI_sakray/USER_AI/AzzyUtil.lua")
dofile("./AI_sakray/USER_AI/Stubs.lua")
dofile("./AI_sakray/USER_AI/A_Friends.lua")
dofile("./AI_sakray/USER_AI/H_Config.lua")
dofile("./AI_sakray/USER_AI/H_Tactics.lua")
dofile("./AI_sakray/USER_AI/KimiNavigation.lua")
dofile("./AI_sakray/USER_AI/AI_main.lua")
dofile("./AI_sakray/USER_AI/H_PVP_Tact.lua")
dofile("./AI_sakray/USER_AI/H_Avoid.lua")
dofile("./AI_sakray/USER_AI/H_Extra.lua")

--########################################
-- CUSTOM ERROR LOGGING SYSTEM
--########################################
-- Comprehensive logging that writes ALL logs to USER_AI/ErrorLog/ folder
-- This overrides the default game client TraceAI function

local logFolder = "./AI_sakray/USER_AI/ErrorLog/"
local errorLogFile = logFolder .. "AAI_ERROR.log"

-- Ensure error log directory exists
local function EnsureLogDir()
	-- Create the ErrorLog directory if it does not exist
	local f = io.open(errorLogFile, "a")
	if f then
		f:close()
		return
	end
	-- Attempt to create directory (Windows-friendly)
	os.execute("mkdir \"" .. logFolder .. "\"")
	-- Try again to open to confirm creation
	local g = io.open(errorLogFile, "a")
	if g then g:close() end
end

-- Main logging function - writes to ERROR log with timestamp
function TraceAI(message)
	if EnableDebugLogging ~= 1 then return end
	-- Write to error log in ErrorLog folder
	local logFile = io.open(errorLogFile, "a")
	if logFile then
		logFile:write(os.date("[%Y-%m-%d %H:%M:%S] ") .. tostring(message) .. "\n")
		logFile:close()
	else
		-- Fallback: write to root folder if ErrorLog doesn't work
		local fallback = io.open("./AI_sakray/USER_AI/KimiAI_Debug.log", "a")
		if fallback then
			fallback:write(os.date("[%Y-%m-%d %H:%M:%S] ") .. tostring(message) .. "\n")
			fallback:close()
		end
	end
end

-- Category-specific logging (uses LogEnable table from H_Extra.lua)
-- Writes to both error log and category-specific log files
function LogToCategory(category, message)
	if EnableDebugLogging ~= 1 then return end
	if LogEnable and LogEnable[category] == 1 then
		-- Write to general error log
		TraceAI("[" .. category .. "] " .. message)
		
		-- Also write to category-specific log file
		local categoryLogFile = logFolder .. "AAI_" .. category .. ".log"
		local catFile = io.open(categoryLogFile, "a")
		if catFile then
			catFile:write(os.date("[%Y-%m-%d %H:%M:%S] ") .. message .. "\n")
			catFile:close()
		end
	end
end

EnsureLogDir()
TraceAI("=== KimiAI Startup ===")

function WriteStartupLog(Version,ErrorCode,ErrorInfo)
	local verspattern="%d+%.%d+"
	if AUVersion==nil then
		AUVersion="1.30b or earlier"
		ErrorCode="File version error"
		ErrorInfo=ErrorInfo.."AzzyUtil.lua no version found"
	elseif string.gfind(AUVersion,verspattern)()~="1.6" then
		ErrorCode="File version error"
		ErrorInfo=ErrorInfo.."AzzyUtil.lua wrong version "..tostring(AUVersion).."\n"
	end
	TestFile=io.open("./AI_sakray/USER_AI/data/test.txt","w")
	if TestFile~=nil then
		TestFile:close()
	else
		ErrorCode="cannot create files in data folder"
		ErrorInfo=ErrorInfo.." Data folder likely missing. Create folder named 'data' in USER_AI "
	end
	if CVersion==nil then
		CVersion="1.30b or earlier"
		ErrorCode="File version error"
		ErrorInfo=ErrorInfo.."Const_.lua no version found"
	elseif string.gfind(CVersion,verspattern)()~="1.7" then
		ErrorCode="File version error"
		ErrorInfo=ErrorInfo.."Const_.lua wrong version "..tostring(CVersion).."\n"
	end
	if MainVersion==nil then
		MainVersion="1.30b or earlier"
		ErrorCode="File version error"
		ErrorInfo=ErrorInfo.." AI_main.lua no version found"
	elseif string.gfind(MainVersion,verspattern)()~="1.7" then
		ErrorCode="File version error"
		ErrorInfo=ErrorInfo.."AI_main.lua wrong version "..tostring(MainVersion).."\n"
	end
	--[[
	if fsize("./AI_sakray/USER_AI/AzzyUtil.lua")~=AULen then
		ErrorCode="Damaged File"
		ErrorInfo=ErrorInfo.." AzzyUtil.lua has been modified or is corrupted"
	end
	if fsize("./AI_sakray/USER_AI/Const_.lua")~=ConstLen then
		ErrorCode="Damaged File"
		ErrorInfo=ErrorInfo.." Const_.lua has been modified or is corrupted"
	end
	if fsize("./AI_sakray/USER_AI/AI_main.lua")~=MainLen then
		ErrorCode="Damaged File"
		ErrorInfo=ErrorInfo.." AI_main.lua has been modified or corrupted"
	end
	--]]
	if fsize("./AI_sakray/USER_AI/H_Config.lua")==3898 then
		ConfigVers="Default: "..LastSavedDate
	else
		ConfigVers="Custom, edited "..LastSavedDate
	end
	if fsize("./AI_sakray/USER_AI/H_Tactics.lua")==5393 then
		TacticVers="Default: "..TactLastSavedDate
	else
		TacticVers="Custom, edited "..TactLastSavedDate
	end
	local OutString
	if ErrorCode=="" then
		OutString="AzzyAI (hom) version "..Version.."\nMain version:"..MainVersion.."\nAzzyUtil version:"..AUVersion.."\nConstant version:"..CVersion.."\nConfig: "..ConfigVers.."\nTactics: "..TacticVers.." \nTime: "..os.date("%c").."\nLua Version:".._VERSION.."\nstarted successfully. \n This AI is installed properly"
	else
		OutString="AzzyAI (hom) version "..Version.."\nMain version:"..MainVersion.."\nAzzyUtil version:"..AUVersion.."\nConstant version:"..CVersion.."\nConfig: "..ConfigVers.."\nTactics: "..TacticVers.." \nTime: "..os.date("%c").."\nLua Version".._VERSION.."\nError: "..ErrorCode.." "..ErrorInfo
	end
	OutFile=io.open("AAIStartH.txt","w")
	if OutFile == nil then
		Error("No write permissions for RO folder, please fix permissions on the RO folder in order to use AzzyAI. Version Info: "..OutString)
	else
		OutFile:write(OutString)
		OutFile:close()
	end
end

function fsize(file)
	local f=io.open(file,"r")
	local size = f:seek("end")
	f:close()
	return size
end

WriteStartupLog(Version,ErrorCode,ErrorInfo)
-------------------------------
-- Add no code to this file
---------------------------------