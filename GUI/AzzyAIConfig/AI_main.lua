...
ComboState = {
	...
	castCountRemaining = 0, -- How many more times to cast current skill
...
function InitCombo()
	...
	ComboState.castCountRemaining = ComboSlot1_CastCount
	ComboState.lastTarget = MyEnemy
	ComboState.lastCycleTime = GetTick()
	TraceAI("Combo initialized: Starting slot 1")
end
...
function ResetCombo()
	...
	ComboState.castCountRemaining = ComboSlot1_CastCount
	ComboState.lastCycleTime = GetTick()
	TraceAI("Combo reset to slot 1")
end

-- Get skill ID and cast count for current slot
function GetComboSlotInfo(slot)
	if slot == 1 then
		return ComboSlot1_SkillID, ComboSlot1_CastCount
	elseif slot == 2 then
		return ComboSlot2_SkillID, ComboSlot2_CastCount
	elseif slot == 3 then
		return ComboSlot3_SkillID, ComboSlot3_CastCount
	elseif slot == 4 then
		return ComboSlot4_SkillID, ComboSlot4_CastCount
	end
	return 0, 0
end
...
function AdvanceComboSlot()
	if ComboState.currentSlot < 4 then
		ComboState.currentSlot = ComboState.currentSlot + 1
	else
		ComboState.currentSlot = 1  -- Wrap back to start
	end
	local skillid, castcount = GetComboSlotInfo(ComboState.currentSlot)
	ComboState.castCountRemaining = castcount
	ComboState.lastCycleTime = GetTick()
	TraceAI("Combo advanced to slot "..ComboState.currentSlot)
end
...
function CanExecuteComboInState()
	if MyState == ATTACK_ST and ComboRunDuringAttack == 1 then
		return true
	elseif MyState == CHASE_ST and ComboRunDuringChase == 1 then
		return true
...
function ExecuteCombo(myid, target)
	...
	-- Check if we're still in skill casting/cooldown from previous action
	if GetTick() < AutoSkillTimeout then
		return  -- Wait for current skill to finish
	end
	
	-- Get current slot skill info
	local skillid, castcount = GetComboSlotInfo(ComboState.currentSlot)
	...
	if AutoSkillCooldown[skillid] and GetTick() < AutoSkillCooldown[skillid] then
		TraceAI("Combo: Skill "..skillid.." on cooldown, waiting...")
		return
	end
	...
	if skilllevel == 0 then
		TraceAI("Combo: Skill "..skillid.." not configured in SkillList for Kimi type "..tostring(KIMITYPE))
		AdvanceComboSlot()
		return
	end
	...
	if GetV(V_SP, myid) < spcost then
		TraceAI("Combo: Not enough SP for skill "..skillid.." (need "..spcost..", have "..GetV(V_SP, myid)..")")
		return
	end
	
	-- Execute the skill - ONLY decrement counter on successful skill execution
	TraceAI("Combo executing: Slot "..ComboState.currentSlot.." Skill "..skillid.." Level "..skilllevel.." (cast "..ComboState.castCountRemaining.." of "..castcount..")")
	DoSkill(skillid, skilllevel, target)
	
	-- Decrement cast count ONLY after successful DoSkill call
	-- This prevents the counter from advancing if skill fails to cast
	ComboState.castCountRemaining = ComboState.castCountRemaining - 1
	
	-- If done with this slot, advance to next
	if ComboState.castCountRemaining <= 0 then
		AdvanceComboSlot()
	else
		-- Update timing for next cast in same slot
		ComboState.lastCycleTime = GetTick()
	end
end
...
function	OnCHASE_ST ()
	...
	if (GetTact(TACT_CHASE,MyEnemy)~=1) then
		...
		-- Enhanced pathfinding: navigate around obstacles/walls
		if ChaseGiveUpCount >= 3 then
			local xcur,ycur = GetV(V_POSITION,MyID)
			local owner = GetV(V_OWNER, MyID)
			local ox,oy = GetV(V_POSITION, owner)
			local movebound = GetMoveBounds()
			
			-- Build candidate list: immediate neighbors + perpendicular reach
			local candidates = {
				-- Orthogonal (prefer straight paths)
				{xcur+1, ycur, 1}, {xcur-1, ycur, 1}, {xcur, ycur+1, 1}, {xcur, ycur-1, 1},
				-- Diagonal (good for corners)
				{xcur+1, ycur+1, 2}, {xcur+1, ycur-1, 2}, {xcur-1, ycur+1, 2}, {xcur-1, ycur-1, 2},
				-- Extended reach for better pathfinding around walls (2 cells away)
				{xcur+2, ycur, 3}, {xcur-2, ycur, 3}, {xcur, ycur+2, 3}, {xcur, ycur-2, 3},
				{xcur+2, ycur+1, 3}, {xcur+2, ycur-1, 3}, {xcur-2, ycur+1, 3}, {xcur-2, ycur-1, 3},
				{xcur+1, ycur+2, 3}, {xcur-1, ycur+2, 3}, {xcur+1, ycur-2, 3}, {xcur-1, ycur-2, 3}
			}
			
			local bestx, besty = xcur, ycur
			local bestDist = GetDistanceA(MyID, MyEnemy)
			local bestPriority = 999
			
			-- Evaluate all candidates, preferring closer distance and better priority
			for _, posDat in ipairs(candidates) do
				local cx, cy, priority = posDat[1], posDat[2], posDat[3]
				
				-- Must stay within owner's movement bounds
				if GetDistanceAPR(owner, cx, cy) <= movebound then
					-- Distance to target
					local d = math.sqrt((cx - ex)*(cx - ex) + (cy - ey)*(cy - ey))
					
					-- Prefer moves that reduce distance, then by priority (orthogonal > diagonal > extended)
					-- If distance improves, pick it; if same distance, prefer higher priority (lower value = better)
					if d < bestDist or (d == bestDist and priority < bestPriority) then
						bestDist = d
						bestx, besty = cx, cy
			...
			-- Execute detour if we found a better position
			if bestx ~= xcur or besty ~= ycur then
				TraceAI("CHASE_ST: Detour to "..bestx..","..besty.." (dist="..math.floor(bestDist*10)/10..")")
				Move(MyID, bestx, besty)
				return
			end
		end
...
function OnATTACK_ST ()
	...
	-- HARD COMBO GATING: when combo is running, do not select or cast any non-combo skills
	if ComboEnabled == 1 and ComboState and ComboState.currentSlot and ComboState.currentSlot > 0 and ComboRunDuringAttack == 1 then
		TraceAI("ATTACK_ST: Combo active, skipping auto skill selection")
...
function DoAutoBuffs(buffmode)
	-- Combo gating: only when a combo is actually active (currentSlot > 0)
	if ComboEnabled == 1 and ComboState and ComboState.currentSlot and ComboState.currentSlot > 0 then
		if (MyState == ATTACK_ST and ComboRunDuringAttack == 1)
			or (MyState == CHASE_ST and ComboRunDuringChase == 1)
			or (MyState == IDLE_ST and ComboRunDuringIdle == 1) then
			-- Skip buffs while combo is actively running in this state
			return 1
		end
	end
	if GetTick() < AutoSkillTimeout then
...
function AI(myid)
	...
	-- Execute combo if enabled and in appropriate context
	-- IMPORTANT: Check state BEFORE calling ExecuteCombo to prevent idle skill spam
	if ComboEnabled == 1 and MyEnemy ~= 0 then
		if CanExecuteComboInState() then
			ExecuteCombo(MyID, MyEnemy)
		end
	end
