...
//
// Description:
// This file contains partial code for the class KimiTactControl, which is
// used as the graphical user interface component for the Kimi tactics
// configurations.
...
{
    public partial class KimiTactControl : UserControl
    {
        // The default Kimi tactics file
        string _file = System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "H_Tactics.lua");

        // An event handler for when the tactics have been changed
        public event EventHandler TacticsChanged;

        public KimiTactControl()
        {
            ...
            // Load the Kimi tactics from the file

            // Pregenerated designer code
            InitializeComponent();
        }

        public KimiTactControl(string file)
        {
            // Check if the file does not exist
            if (!File.Exists(file))
            {
